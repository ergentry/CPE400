...
public class Model extends UndirectedSparseMultigraph<Router, Link> implements MessageFactory
{
	...
	static class LinkFactoryImpl implements LinkFactory
	{
		...
		public Link create()
		{
			final Link link = LinkImple.newInstance().setModel(this.model).setId(LinkFactoryImpl.nextSerialNumber.incrementAndGet()).build();
			// link.start();
			return link;
		}
	...
	static class MessageFactoryImpl implements MessageFactory
	{
		...
		public Message newControlMessage(final Router source, final Router destination)
		{
			...
			final Message message = MessageImple.newInstance().setId(id).setSource(source).setDestination(destination).setTtl(this.timeToLive).build();
		...
		{
			final int id = MessageFactoryImpl.nextSerialNumber.getAndIncrement();
			final Message message = MessageImple.newInstance().setId(id).setSource(source).setDestination(destination).setTtl(this.timeToLive).setLength(size).build();
			if (this.messageListener != null)
			{
				message.addMessageListener(this.messageListener);
			}
	...
	static class RouterFactoryImpl implements RouterFactory
	{
		...
		{
			final Router node = RouterImple.newInstance().setModel(this.model).setId(RouterFactoryImpl.nextSerialNumber.getAndIncrement()).build();
			// node.start();
			return node;
		}
